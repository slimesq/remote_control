...
连接测试、磁盘、目录、打开和删除属于独立请求，客户端为每个请求创建一个异步短连接。
屏幕帧和控制命令需要持续交互，因此分别使用两条长连接。下载持续时间较长，但一次下载具有明确
终点，所以使用专用的一次性连接和常驻下载 worker。

这样拆分的主要目的不是增加连接数量，而是隔离不同业务的延迟。例如，大尺寸 PNG 或慢速文件
下载不会占用鼠标控制通道；控制命令也不会插入文件数据流。

### GUI 线程避免同步等待

客户端有一个 GUI 线程以及屏幕流、控制流和下载三个常驻工作线程。每个 worker 只在自己的线程
操作所属的 `QTcpSocket`、定时器和文件对象。GUI 通过 `Qt::QueuedConnection` 投递请求，并通过
signal 接收结果，因此网络等待和文件写入不会阻塞窗口事件循环。

销毁客户端时，每个 worker 在同一个队列回调中先执行 `shutdown()`，再退出自己的事件循环；
析构线程随后 `wait()` 回收线程。这既保持 QObject 的线程归属，也避免把清理与退出拆成两次阻塞
操作所产生的停机时序窗口。

`OneShotRequest` 位于 GUI 线程，但其 `QTcpSocket` 仍是事件驱动的异步 socket，不会在请求期间
同步阻塞 GUI。项目还用 `CallbackScope` 延迟对象清理，避免模态对话框启动嵌套事件循环时，
`disconnected` 回调在外层网络回调返回前提前清理请求对象。

异步 socket 不等于所有处理都离开 GUI 线程：一次性请求的 Packet 解析和目录条目累计仍在 GUI
事件回调中完成。普通规模目录不会同步等待网络，但超大目录仍可能带来短时处理负载；如果以后需要
面向超大目录，应再把解析与模型更新拆成可分批调度的工作。

### 状态机代替互相关联的布尔标志

项目在多处使用枚举或单向状态机表达生命周期：

- 目录使用 `Unloaded`、`Loading`、`Loaded`、`Refreshing` 表示缓存与请求状态。
- 下载、屏幕 worker 和控制 worker 都有各自的生命周期枚举。
- 服务端连接从 `AwaitingRequest` 单向分类为一种业务角色，最终经过 `Closing` 到达 `Closed`。
- 服务端屏幕流控记录空闲、单帧在途以及已有一个合并请求等状态。

这样可以明确合法转换，避免多个布尔值组合出没有业务含义的状态。

### generation 过滤过期异步结果

网络结果可能在用户切换 endpoint、关闭监控窗口或启动下一次操作之后才回到 GUI。客户端分别维护
endpoint、download、screen stream 和 control stream generation。请求开始时捕获 generation，
回调到达时只有仍与当前值匹配才会被转发。

generation 解决的是“结果是否还属于当前操作”，它不能取消已经发生的远程副作用。例如，旧的
删除请求即使结果被丢弃，服务端仍可能已经完成删除。

### 有界队列和背压

客户端控制队列和服务端任务池、发送队列都具有容量限制。服务端每条连接只允许一个
`WSASend` 在途，目录和下载按照发送完成节奏继续生成下一批数据。慢客户端不会让 worker 阻塞等待，
也不能无限制积累待发送数据；超过限制时连接会被关闭。
